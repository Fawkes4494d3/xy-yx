...
for MJ in range(N):
	...
	Centers=random.sample(range(n),2*k)
	for i in range(2*k):
	...
	Member=[Distances[i].index(min(Distances[i])) for i in range(n)]
	for i in Member:
		cluster_size[i]+=1
	centers=[]
	for i in range(k):
		centers.append(Centers[cluster_size.index(max(cluster_size))])
		Centers.remove(Centers[cluster_size.index(max(cluster_size))])
		cluster_size.remove(max(cluster_size))
	distances=[]
	for i in range(n):
		M=[]
		for j in range(k):
